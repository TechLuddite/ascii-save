...
- The active local default (2026-09-14) is the **colour Giants trial**: the user-owned revised-draft copy with `scripts/omarchy-screensaver-color` and `scripts/omarchy-screensaver-prepare-color` installed in its `source/bin/`, playing `examples/giants-color` (truecolour quadrant blocks). Read the current-state sections of `docs/CONTEXT.md` and `docs/LOCAL-INTEGRATION.md` first. Do not overwrite those two files with the PR versions as a "repair"; the backups and hash record are described there.
...
- The user has asked for **no changes to the draft PR** until the colour work has had a code-burden and code-security review. Neither has happened. Do not touch omacom/omarchy#11626. Keep colour work in this repository and the local trial copy.
