...
- The active local default (2026-09-14) is the **colour Giants trial**: the user-owned revised-draft copy now running the PR's own renderer, preparer and importer at `667b14c`, playing `examples/giants-color` (truecolour quadrant blocks) with the voted `screensaver.effects` list in shell.json. Read the current-state sections of `docs/CONTEXT.md` and `docs/LOCAL-INTEGRATION.md` first. Do not overwrite those two files with the PR versions as a "repair"; the backups and hash record are described there.
- Colour artwork carries SGR sequences. The pinned reference preparer and `scripts/validate.py` without `--color` reject it by design; use `--color` or `common.COLOR_PREPARER`. Never widen the preparer beyond `ESC [ digits ; m`.
- The colour renderer's `include_effects` list is the user's vote, made with `curation/curate.py` and recorded under `curation/votes/`. Do not hand-edit it; changing the rotation means recording, voting and `apply` again.
- The draft PR omacom/omarchy#11626 was updated to `667b14c` on 2026-09-14 after an adversarial review the user required. Further PR changes need the same: review first, then push, and record the findings in `docs/CONTEXT.md`. Live checks of the screensaver must run from a script file (never a command line containing the window class) and must verify the active window before sending keys.
